...
<<<<<<< HEAD
	...
	const auto add_signed_mod = [modulus_q](uint64_t value, int64_t noise) {
		if (noise >= 0)
			return (value + static_cast<uint64_t>(noise) % modulus_q) % modulus_q;
		const uint64_t magnitude =
			static_cast<uint64_t>(-(noise + 1)) + 1;
		return (value + modulus_q - magnitude % modulus_q) % modulus_q;
	};
