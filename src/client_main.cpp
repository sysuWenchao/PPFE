...
<<<<<<< HEAD
    ...
    if (use_sequential_queries)
    {
        for (uint32_t i = 0; i < num_queries; ++i)
            query_indices.push_back(i % N);
    }
    else
    {
        // Benchmark random queries without replacement. Repeated queries
        // exercise a separate hint-reuse path and are not part of this sweep.
        std::unordered_set<uint32_t> selected_queries;
        while (query_indices.size() < static_cast<size_t>(num_queries))
        {
            const uint32_t candidate = static_cast<uint32_t>(
                SecureRandomUint64Below(static_cast<uint64_t>(N)));
            if (selected_queries.insert(candidate).second)
