...
namespace troy
{
    namespace utils
    {
        ...
        void fill_uint128s(Slice<uint8_t> bytes, ruint128_t seed, ruint128_t &counter)
        {
           ...
           /*
                __host__ __device__ static int uint64_to_cbd(uint64_t x_uint64)
                {
                    uint8_t *x = reinterpret_cast<uint8_t *>(&x_uint64);
                    x[2] &= 0x1f;
                    x[5] &= 0x1f;
                    return utils::hamming_weight(x[0]) + utils::hamming_weight(x[1]) + utils::hamming_weight(x[2]) - utils::hamming_weight(x[3]) - utils::hamming_weight(x[4]) - utils::hamming_weight(x[5]);
                }*/
        __host__ __device__ static int uint64_to_cbd(ruint128_t rf)
        {
            uint64_t lo = rf.low;
            uint64_t hi = rf.high;

            int pos = utils::hamming_weight(lo & 0x3FFFFFFFFULL); // 34 bits
            int neg = utils::hamming_weight(hi & 0x3FFFFFFFFULL); // 34 bits
        ...
        void RandomGenerator::fill_bytes_batched(const SliceVec<uint8_t> &bytes, MemoryPoolHandle pool)
        {
            ...
            if (main > 0)
            {
                SliceVec<uint8_t> bytes_main;
                bytes_main.reserve(bytes.size());
                for (size_t i = 0; i < bytes.size(); i++)
                {
        ...
        void RandomGenerator::fill_uint64s_batched(const SliceVec<uint64_t> &uint64s, MemoryPoolHandle pool)
        {
            if (uint64s.size() == 0)
                return;
            SliceVec<uint8_t> reinterpreted;
            reinterpreted.reserve(uint64s.size());
            ...
            ruint128_t seed, ruint128_t counter)
        {
            ...
            if (idx < degree)
            {
                ...
                for (size_t index = idx; index < destination.size(); index += degree)
                {
                    if (r == 2)
                    {
                        destination[index] = moduli[i].value() - 1; // 三元分布实现
                    }
                    else
                    {
                        destination[index] = r;
        ...
        */
        ...
        void RandomGenerator::sample_poly_ternary(Slice<uint64_t> destination, size_t degree, ConstSlice<Modulus> moduli)
        {
            ...
            if (!device)
            {
                for (size_t j = 0; j < degree; j++)
                {
                    // 每个系数独立使用一个 128-bit 随机数
                    ruint128_t rf = host_generate_uint128(this->seed, this->counter);
                    // CBD(η=34) 采样，返回有符号整数
                    int s = uint64_to_cbd(rf);

                    for (size_t i = 0; i < moduli.size(); i++)
                    {
                        size_t index = i * degree + j;
                        uint64_t qi = moduli[i].value();

                        // 映射到 Z_q
                        if (s >= 0)
                        {
                            destination[index] = static_cast<uint64_t>(s);
                        }
                        else
                        {
                            destination[index] = qi - static_cast<uint64_t>(-s);
                        }
                    }
                }
            }
